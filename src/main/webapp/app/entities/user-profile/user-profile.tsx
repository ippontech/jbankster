    ...
    return (
      <div>
        <h2 id="user-profile-heading">
          <Translate contentKey="jbanksterApp.userProfile.home.title">User Profiles</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="jbanksterApp.userProfile.home.createLabel">Create new User Profile</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {userProfileList.map((userProfile, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {userProfile.stellarAccounts
                      ? userProfile.stellarAccounts.map((val, j) => (
                          <span key={j}>
                            <Link to={`stellarAccount/${val.id}`}>{val.id}</Link>
                            {j === userProfile.stellarAccounts.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
